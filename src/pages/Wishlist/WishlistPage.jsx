...
function WishlistItemCard({ item, onRemove, isRemoving }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded border border-advika-border-light bg-white" data-testid={`wishlist-item-${item.productId}`}>
      <div className="relative flex h-[132px] items-center justify-center bg-advika-ink">
        ...
        {(wattage || voltage.hasVoltage) && (
          <div className="absolute left-2 top-2 z-10 flex gap-1">
            ...
            {voltage.hasVoltage && (
              <span
                className={`aa-mono rounded-sm px-[6px] py-[3px] text-[9px] font-semibold text-white ${
      ...
      <div className="flex flex-col gap-2 p-3">
        {category && <span className="aa-label text-[9px] font-semibold text-advika-orange-dark">{t(`advika.category.${category.id}`)}</span>}
        <Link to={buildProductPath(product, name)} className="min-h-[36px] text-[13.5px] font-bold leading-[1.3] text-advika-chrome line-clamp-2">
          {name}
        </Link>
        ...
        <div className="flex items-center gap-[5px] text-[10.5px] font-semibold">
          {!stock.available ? (
        ...
        <button
          ...
          className={`aa-tracking flex h-11 items-center justify-center gap-[6px] rounded-[3px] text-[11.5px] font-bold text-white ${
...
export default function WishlistPage() {
  ...
  return (
    <div className="aa-shell aa-page-wishlist min-h-screen bg-advika-warm-white">
      ...
      <main id="main-content" tabIndex={-1}>
        {isSyncing ? (
          ...
          <div className="flex flex-col items-center gap-4 px-6 pb-[60px] pt-[52px] text-center" data-testid="wishlist-empty-state">
            ...
            <h1 className="font-archivoBlack text-[21px] leading-[1.2] text-advika-chrome">{t('advika.wishlistPage.emptyTitle')}</h1>
            <p className="max-w-[290px] text-[13.5px] text-advika-grey800">{t('advika.wishlistPage.emptyBody')}</p>
            <button
              type="button"
              onClick={() => navigate('/products')}
              data-testid="wishlist-browse-products-button"
              className="aa-tracking mt-[6px] flex h-[54px] items-center justify-center gap-[9px] rounded bg-advika-orange px-[26px] text-[14px] font-bold text-white"
            >
              {t('advika.wishlistPage.browseProducts')} <span>→</span>
            </button>
          </div>
        ) : (
          <>
            <div className="flex flex-col gap-4 bg-advika-near-black px-4 pb-[22px] pt-6">
              ...
              <div className="flex items-start gap-[12px]">
                ...
                <div className="flex flex-col gap-[5px]">
                  ...
                  <p className="aa-label text-[11.5px] text-advika-grey600">{t('advika.wishlistPage.savedItems', { count: items.length })}</p>
                </div>
              </div>
              <button
                type="button"
                ...
                className="aa-tracking flex h-[52px] items-center justify-center gap-[9px] rounded bg-advika-orange text-[13.5px] font-bold text-white disabled:opacity-60"
              >
                <Icon name="shopping_cart" size={19} /> {t('advika.wishlistPage.addAllToCart')}
              </button>
            </div>
